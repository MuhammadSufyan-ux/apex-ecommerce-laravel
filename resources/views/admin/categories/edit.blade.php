...
<div class="mb-10">
    ...
    <h2 class="text-2xl font-black text-black uppercase tracking-tighter text-2xl">Modify Collection</h2>
</div>

<form action="{{ route('admin.categories.update', $category->id) }}" method="POST" class="max-w-4xl">
    @csrf
    @method('PUT')
    <div class="bg-white p-10 border border-gray-100 shadow-sm space-y-8">
        <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
            <div class="space-y-2">
                ...
                <input type="text" name="name" value="{{ old('name', $category->name) }}" required class="w-full bg-gray-50 border border-gray-200 px-4 py-3 text-sm font-bold tracking-tight focus:border-gold focus:ring-0">
            </div>
        ...
        <div class="space-y-4 pt-6 border-t border-gray-50">
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                <label class="flex items-center gap-3 cursor-pointer group">
                    <input type="checkbox" name="show_in_menu" {{ $category->show_in_menu ? 'checked' : '' }} class="w-5 h-5 text-gold border-gray-300 focus:ring-gold">
                    <span class="text-[10px] font-bold uppercase tracking-widest text-gray-600 group-hover:text-black">Show in Main Menu</span>
                </label>
                
                <label class="flex items-center gap-3 cursor-pointer group">
                    <input type="checkbox" name="show_in_featured" {{ $category->show_in_featured ? 'checked' : '' }} class="w-5 h-5 text-gold border-gray-300 focus:ring-gold">
                    <span class="text-[10px] font-bold uppercase tracking-widest text-gray-600 group-hover:text-black">Shop by Category</span>
                </label>

                <label class="flex items-center gap-3 cursor-pointer group">
                    <input type="checkbox" name="is_active" {{ $category->is_active ? 'checked' : '' }} class="w-5 h-5 text-gold border-gray-300 focus:ring-gold">
